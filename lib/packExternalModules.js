...
function someFileExists(paths) {
  return BbPromise.all(paths.map(x => (
    BbPromise.fromCallback(cb => fse.pathExists(x, cb))
  ))).then(results => {
    const existsFiles = _.zip(paths, results)
      .filter(([_, exists]) => exists);
    return existsFiles[0][0];
  });
}
  ...
  packExternalModules() {
    ...
    return BbPromise.try(() => Packagers.get.call(this, this.configuration.packager)).then(packager => {
      ...
      return packager.getProdDependencies(path.dirname(packageJsonPath), 1).then(dependencyGraph => {
        ...
        const packageLockPathFallBack = path.join(path.dirname(packageJsonPath), '../../', packager.lockfileName);
        let hasPackageLock = false;
        return someFileExists([packageLockPath, packageLockPathFallBack])
          .then(lockPath => {
            if (lockPath) {
              this.serverless.cli.log('Package lock found - Using locked versions');
              try {
                let packageLockFile = this.serverless.utils.readFileSync(lockPath);
